...
(() => {
  ...
  function openEditor(type='note'){
    ...
    setText('editorLabel',type==='note'?'NOTE':'TASK');
  ...
  function saveEditor(){
    ...
    S.customReminders.push({id:'custom-'+Date.now(),text:title,time:$('editorTime').value||'10:00',mode:$('editorMode').value,repeat:'once',enabled:true,note:text});
  ...
  $('addCustomReminder')?.addEventListener('click',()=>{
    ...
    S.customReminders.push({id:'custom-'+Date.now(),text,time:$('customTaskTime').value||'10:00',mode:$('customTaskMode').value,repeat:$('customTaskRepeat').value,enabled:true});
